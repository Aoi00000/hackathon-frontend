...
import { formatDate, normalizeImageUrl, purchaseStatusLabel, statusLabel } from '../utils';
...
export function ItemDetailPage() {
  ...
  return <section className="detail">
    <div className="card">
      ...
      <dl className="meta"><dt>独自商品ID</dt><dd>{item.productCode}</dd><dt>カテゴリ</dt><dd>{item.category}</dd><dt>状態</dt><dd>{item.conditionText}</dd><dt>価格</dt><dd>¥{item.priceYen.toLocaleString()}</dd><dt>送料</dt><dd>送料無料</dd><dt>商品の受け渡し方法</dt><dd>{item.deliveryMethod}</dd><dt>発送までの日数</dt><dd>{item.shippingDays}日</dd><dt>発送元の地域</dt><dd>{item.shipFromRegion}</dd><dt>サイズ</dt><dd>{item.size || '-'}</dd><dt>色</dt><dd>{item.color || '-'}</dd><dt>タグ</dt><dd>{item.tags || '-'}</dd><dt>ステータス</dt><dd><span className={`statusText ${item.status}`}>{statusLabel(item.status)}</span></dd><dt>最終更新日時</dt><dd>{formatDate(item.updatedAt)}</dd><dt>出品者評価</dt><dd>{item.sellerRatingCount ? `${item.sellerRatingAverage.toFixed(1)} / 5 (${item.sellerRatingCount}件)` : '評価なし'}</dd><dt>取引実績</dt><dd>{item.sellerTransactionCount}件</dd></dl>
