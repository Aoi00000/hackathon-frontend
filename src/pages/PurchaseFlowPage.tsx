...
export function PurchaseFlowPage() {
  ...
  return <section className="stack">
    ...
    <div className="card purchaseLayout"><div>{item.imageUrl && <img className="thumb" src={normalizeImageUrl(item.imageUrl)} alt={item.title} />}<h2>{item.title}</h2><p>{item.productCode}</p><strong>¥{item.priceYen.toLocaleString()}</strong><p className="muted">送料: 送料無料</p></div><div><h2>残高</h2><p className={insufficient ? 'error' : 'success'}>現在の残高: {latestUser.balanceCoins.toLocaleString()} コイン</p><p>購入後に、購入者の残高から商品代金が引かれ、出品者の売上金へ移ります。</p></div></div>
