...
/**
 * 実装詳細メモ:
 * 自分が出品した商品を販売中・取引中・完了済みに分けて管理します。
 * 編集フォームはカード内に閉じ込め、更新後に一覧を再取得してDB上の販売状態と画面を一致させます。
 */
...
// EditableItemCard は、出品した商品1件の表示・編集・キャンセル・発送通知をまとめたカードです。
// 販売中だけ編集可能にし、購入済み商品では取引状態確認と発送通知に役割を絞ります。
...
// MyItemsPage は、ログインユーザーの出品履歴をAvailable/SOLDに分けて管理する画面です。
// 出品の編集、出品キャンセル、発送通知をここから行い、操作後は一覧を再取得してDB状態と同期します。
